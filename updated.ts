...
export class Board {
  ...
  public findAllPossibleMoves(tileToMove: GameTile): Move[] {
    ...
    if (pieceToMove) {
      ...
      const { x, y }: Coordinates = tileToMove;

      const jumps = this.findAllJumps({ x, y }, this.board, directionOfMotion[0], [], [], false, pieceToMove.getBelongsTo());
  ...
  private canJump(coordinates: Coordinates, directions: number[], leftOrRight: number[]) {
    const { x, y } = coordinates;
    return this.board[y + directions[k]] &&
  ...
  public findAllJumps(coordinates: Coordinates, board: GameTile[][], directionOfMotion: number, possibleJumps: Move[], wouldDelete: Coordinates[], isKing: boolean, activePlayer: Player): Move[] {
    ...
    for (let direction = 0; direction < directions.length; direction++) {
      for (let side = 0; side < leftOrRight.length; side++) {
        if (this.canJump(coordinates, directions, leftOrRight)) {
    ...
    if (thisIterationDidSomething) {
			for (let i = 0; i < possibleJumps.length; i++) {
				...
				var foundJumps = this.findAllJumps(coords[0], coords[1], board, directionOfMotion, possibleJumps, possibleJumps[i].wouldDelete, isKing, activePlayer);
				for (var j = 0; j < foundJumps.length; j++) {
					if (possibleJumps.indexOf(foundJumps[j]) < 0) {
						possibleJumps.push(foundJumps[j]);
					}
				}
			}
		}
